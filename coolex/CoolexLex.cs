...
{ Type.BLOCK_OPEN, new[] { "(" } },{ Type.BLOCK_CLOSE, new[] { ")" } },{ Type.DCL, new[] { "DCL" } },{ Type.ENDDCL, new[] { "END" } },{ Type.OPERATION, new[] { "EXEC", "ENDIF", "ENDSL", "ENDDO", "ENDFOR", "ENDMON", "ACQ", "ADD", "ADDDUR", "ALLOC", "ANDxx", "BEGSR", "BITOFF", "BITON", "CABxx", "CALL", "CALLB", "CALLP", "CASxx", "CAT", "CHAIN", "CHECK", "CHECKR", "CLEAR", "CLOSE", "COMMIT", "COMP", "DEALLOC", "DELETE", "DIV", "DO", "DOU", "DOUxx", "DOW", "DOWxx", "DSPLY", "DUMP", "ELSE", "ELSEIF", "ENDyy", "ENDSR", "EVAL", "EVALR", "EVAL-CORR", "EXCEPT", "EXFMT", "EXSR", "EXTRCT", "FEOD", "FOR", "FORCE", "GOTO", "IF", "IFxx", "IN", "ITER", "KFLD", "KLIST", "LEAVE", "LEAVESR", "LOOKUP", "MHHZO", "MHLZO", "MLHZO", "MLLZO", "MONITOR", "MOVE", "MOVEA", "MOVEL", "MULT", "MVR", "NEXT", "OCCUR", "ON-ERROR", "ON-EXIT", "OPEN", "ORxx", "OTHER", "OUT", "PLIST", "POST", "READ", "READC", "READE", "READP", "READPE", "REALLOC", "REL", "RESET", "RETURN", "ROLBK", "SCAN", "SELECT", "SETGT", "SETLL", "SETOFF", "SETON", "SHTDN", "SORTA", "SQRT", "SUB", "SUBDUR", "SUBST", "TAG", "TEST", "TESTB", "TESTN", "TESTZ", "TIME", "UNLOCK", "UPDATE", "WHEN", "WHENxx", "WRITE", "XFOOT", "XLATE", "XML-INTO", "XML-SAX", "Z-ADD", "Z-SUB" } },{ Type.BIF, new[] { "/\\%\\S*/" } },{ Type.SPECIAL, new[] { "/\\*\\S*/" } },{ Type.DIRECTIVE, new[] { "/\\/\\S*/" } },{ Type.DOUBLE_LITERAL, new[] { @"/(?<=^| )\\d+(\\.\\d+)?(?=$| )/" } },{ Type.INT_LITERAL, new[] { "/^[-+]?\\d+$/" } },{ Type.WORD_LITERAL, new[] { "/.*?/" } },{ Type.EQUALS, new[] { "=" } },{ Type.PARMS, new[] { ":" } },{ Type.STMT_END, new[] { ";" } },{ Type.DOT, new[] { "." } },{ Type.ADD, new[] { "+" } },{ Type.SUB, new[] { "-" } },{ Type.DIV, new[] { "/" } },{ Type.MUL, new[] { "*" } },{ Type.LESS_THAN, new[] { "<" } },{ Type.MORE_THAN, new[] { ">" } },{ Type.NOT, new[] { "<>" } },{ Type.MT_EQUAL, new[] { ">=" } },{ Type.LT_EQUAL, new[] { "<=" } }
        ...
        public static ParseError[] Parse(CoolexType[] tokens)
        {
            ...
            for (int i = 0; i < tokens.Length; i++)
            {
                ...
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "PI": inner++; if (inner < tokens.Length) { doBreak = false; for (; inner < tokens.Length && doBreak == false;) { switch (tokens[inner].Type) { case Type.MUL: case Type.WORD_LITERAL: case Type.SPECIAL: case Type.ENDDCL: case Type.SUB: case Type.BLOCK:/* all good babies */inner++; break; case Type.STMT_END:  /* all good babies */  doBreak = true; inner--; inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.STMT_END:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected STMT_END")); } break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); doBreak = true; inner++; break; } } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected MUL, WORD_LITERAL, SPECIAL, ENDDCL, SUB, BLOCK")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"PI\"")); }
                ...
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.OPERATION: inner++; if (inner < tokens.Length) { doBreak = false; for (; inner < tokens.Length && doBreak == false;) { switch (tokens[inner].Type) { case Type.WORD_LITERAL: case Type.DOUBLE_LITERAL: case Type.INT_LITERAL: case Type.STRING_LITERAL: case Type.BIF: case Type.BLOCK: case Type.PARMS: case Type.EQUALS: case Type.DOT: case Type.ADD: case Type.SUB: case Type.DIV: case Type.MUL: case Type.LESS_THAN: case Type.MORE_THAN: case Type.NOT: case Type.MT_EQUAL: case Type.LT_EQUAL:/* all good babies */inner++; break; case Type.STMT_END:  /* all good babies */  doBreak = true; inner--; inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.STMT_END:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected STMT_END")); } break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); doBreak = true; inner++; break; } } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, STRING_LITERAL, BIF, BLOCK, PARMS, EQUALS, DOT, ADD, SUB, DIV, MUL, LESS_THAN, MORE_THAN, NOT, MT_EQUAL, LT_EQUAL")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected OPERATION")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "ENDDO": inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.STMT_END:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected STMT_END")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"ENDDO\"")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "ENDFOR": inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.STMT_END:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected STMT_END")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"ENDFOR\"")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "ENDIF": inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.STMT_END:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected STMT_END")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"ENDIF\"")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "BEGSR": case "EXFMT": inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.WORD_LITERAL:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected WORD_LITERAL, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected WORD_LITERAL")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"BEGSR\", \"EXFMT\"")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.ENDDCL: inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.SUB: inner++; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "PI": case "PR": case "PROC": case "DS": inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.STMT_END:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected STMT_END")); } break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected \"PI\", \"PR\", \"PROC\", \"DS\", got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"PI\", \"PR\", \"PROC\", \"DS\"")); } break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected SUB, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected SUB")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected ENDDCL")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.ADD: case Type.SUB: case Type.DIV: case Type.MUL: inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.EQUALS: case Type.WORD_LITERAL: case Type.DOUBLE_LITERAL: case Type.INT_LITERAL: case Type.STRING_LITERAL: case Type.BIF: case Type.BLOCK:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, STRING_LITERAL, BIF, BLOCK, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, STRING_LITERAL, BIF, BLOCK")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected ADD, SUB, DIV, MUL")); }
                ...
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.EQUALS: inner++; if (inner < tokens.Length) { doBreak = false; for (; inner < tokens.Length && doBreak == false;) { switch (tokens[inner].Type) { case Type.WORD_LITERAL: case Type.DOUBLE_LITERAL: case Type.INT_LITERAL: case Type.STRING_LITERAL: case Type.BIF: case Type.BLOCK: case Type.EQUALS: case Type.DOT: case Type.ADD: case Type.SUB: case Type.DIV: case Type.MUL: case Type.LESS_THAN: case Type.MORE_THAN: case Type.NOT: case Type.MT_EQUAL: case Type.LT_EQUAL:/* all good babies */inner++; break; case Type.STMT_END:  /* all good babies */  doBreak = true; inner--; inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.STMT_END:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected STMT_END")); } break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected STMT_END, got " + tokens[inner].Value)); doBreak = true; inner++; break; } } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, STRING_LITERAL, BIF, BLOCK, EQUALS, DOT, ADD, SUB, DIV, MUL, LESS_THAN, MORE_THAN, NOT, MT_EQUAL, LT_EQUAL")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected EQUALS")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "EXEC": inner++; if (inner < tokens.Length) { switch (tokens[inner].Value.ToUpper()) { case "SQL":/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected \"SQL\", got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"SQL\"")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected \"EXEC\"")); }
            }
