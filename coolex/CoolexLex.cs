...
{ Type.BLOCK_OPEN, new[] { "(" } },{ Type.BLOCK_CLOSE, new[] { ")" } },{ Type.DCL, new[] { "DCL" } },{ Type.ENDDCL, new[] { "END" } },{ Type.OPERATION, new[] { "EXEC", "ENDIF", "ENDSL", "ENDDO", "ENDFOR", "ENDMON", "ACQ", "ADD", "ADDDUR", "ALLOC", "ANDxx", "BEGSR", "BITOFF", "BITON", "CABxx", "CALL", "CALLB", "CALLP", "CASxx", "CAT", "CHAIN", "CHECK", "CHECKR", "CLEAR", "CLOSE", "COMMIT", "COMP", "DEALLOC", "DELETE", "DIV", "DO", "DOU", "DOUxx", "DOW", "DOWxx", "DSPLY", "DUMP", "ELSE", "ELSEIF", "ENDyy", "ENDSR", "EVAL", "EVALR", "EVAL-CORR", "EXCEPT", "EXFMT", "EXSR", "EXTRCT", "FEOD", "FOR", "FORCE", "GOTO", "IF", "IFxx", "IN", "ITER", "KFLD", "KLIST", "LEAVE", "LEAVESR", "LOOKUP", "MHHZO", "MHLZO", "MLHZO", "MLLZO", "MONITOR", "MOVE", "MOVEA", "MOVEL", "MULT", "MVR", "NEXT", "OCCUR", "ON-ERROR", "ON-EXIT", "OPEN", "ORxx", "OTHER", "OUT", "PLIST", "POST", "READ", "READC", "READE", "READP", "READPE", "REALLOC", "REL", "RESET", "RETURN", "ROLBK", "SCAN", "SELECT", "SETGT", "SETLL", "SETOFF", "SETON", "SHTDN", "SORTA", "SQRT", "SUB", "SUBDUR", "SUBST", "TAG", "TEST", "TESTB", "TESTN", "TESTZ", "TIME", "UNLOCK", "UPDATE", "WHEN", "WHENxx", "WRITE", "XFOOT", "XLATE", "XML-INTO", "XML-SAX", "Z-ADD", "Z-SUB" } },{ Type.BIF, new[] { "/\\%\\S*/" } },{ Type.SPECIAL, new[] { "/\\*\\S*/" } },{ Type.DIRECTIVE, new[] { "/\\/\\S*/" } },{ Type.DOUBLE_LITERAL, new[] { @"/(?<=^| )\\d+(\\.\\d+)?(?=$| )/" } },{ Type.INT_LITERAL, new[] { "/^[-+]?\\d+$/" } },{ Type.WORD_LITERAL, new[] { "/.*?/" } },{ Type.EQUALS, new[] { "=" } },{ Type.PARMS, new[] { ":" } },{ Type.STMT_END, new[] { ";" } },{ Type.DOT, new[] { "." } },{ Type.ADD, new[] { "+" } },{ Type.SUB, new[] { "-" } },{ Type.DIV, new[] { "/" } },{ Type.MUL, new[] { "*" } },{ Type.LESS_THAN, new[] { "<" } },{ Type.MORE_THAN, new[] { ">" } },{ Type.NOT, new[] { "<>" } },{ Type.MT_EQUAL, new[] { ">=" } },{ Type.LT_EQUAL, new[] { "<=" } }
        ...
        public static ParseError[] Parse(CoolexType[] tokens)
        {
            ...
            for (int i = 0; i < tokens.Length; i++)
            {
                ...
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.ADD: inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.EQUALS: case Type.WORD_LITERAL: case Type.DOUBLE_LITERAL: case Type.INT_LITERAL: case Type.STRING_LITERAL: case Type.BIF: case Type.BLOCK:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, STRING_LITERAL, BIF, BLOCK, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, STRING_LITERAL, BIF, BLOCK")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected ADD")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.SUB: inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.EQUALS: case Type.WORD_LITERAL: case Type.DOUBLE_LITERAL: case Type.INT_LITERAL: case Type.BIF: case Type.BLOCK:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, BIF, BLOCK, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, BIF, BLOCK")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected SUB")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.DIV: inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.EQUALS: case Type.WORD_LITERAL: case Type.DOUBLE_LITERAL: case Type.INT_LITERAL: case Type.BIF: case Type.BLOCK: case Type.STRING_LITERAL:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, BIF, BLOCK, STRING_LITERAL, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, BIF, BLOCK, STRING_LITERAL")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected DIV")); }
                inner = i; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.MUL: inner++; if (inner < tokens.Length) { switch (tokens[inner].Type) { case Type.EQUALS: case Type.WORD_LITERAL: case Type.DOUBLE_LITERAL: case Type.INT_LITERAL: case Type.BIF: case Type.BLOCK:/* all good babies */break; default: errors.Add(new ParseError(tokens[inner].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, BIF, BLOCK, got " + tokens[inner].Value)); inner++; break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected EQUALS, WORD_LITERAL, DOUBLE_LITERAL, INT_LITERAL, BIF, BLOCK")); } break; } } else { errors.Add(new ParseError(tokens[tokens.Length - 1].Line, "Expected MUL")); }
